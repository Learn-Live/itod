...
class BaseDetectorFactory():
    ...
    def run(self):
        ...
        for i, (key, value_dict) in enumerate(self.dataset_inst.dataset_dict.items()):
            ...
            if key in ['samp_num_dict', 'samp_size_dict']:
                samp_best_auc = 0
                samp_set_dict = {}
                result_best_dict = {}
                auc_lst = []
            ...
            #              'stat_dict']:
            elif key in ['iat_dict', 'iat_size_dict', 'stat_dict']:
...
class OCSVMFactory(BaseDetectorFactory):
    ...
    def run_model(self, feat_dict):
        ...
        X_val, X_test, y_val, y_test = train_test_split(X_test, y_test, train_size=100 * 2, test_size=300 * 2,
                                                        stratify=y_test,
                                                        random_state=42)
        print(f'gs: {gs}, X_train: {X_train.shape}, y_train: {Counter(y_train)}')
        if gs: print(f'gs: {gs}, X_val: {X_val.shape}, y_val: {Counter(y_val)}')
