...
#
#     print(f"k: {k}")
#     # Declare a Quickshift++ ndm with tuning hyperparameters.
#     if k == '':
...
def change_scores_to_quantiles_voted_auc(y_testing_scores_arrs, y_training_scores_arrs, y_true, num_quantile=20):
    ...
    for i, p in enumerate(ps):
        ...
        for j in range(y_testing_scores_arrs.shape[0]):
            # process the y_testing_scores generated by the i-th ndm
            ...
            for t, v in enumerate(arr):
                if v >= quant:
                    ...
                    majority_arrs[j][t] = 0
        # get the majority of 0-1 predictions for one ndm for each sample
